...
<p align="center">
  <b>ROS bag を一度デコードすれば、あとはデータベースのようにクエリし、何度でも LeRobot 学習データに変換できます。</b>
</p>
...
ROS bag は *記録* のための形式であって、*活用* のための形式ではありません。サイズが大きく、読み込みが遅く、メッセージ単位で直列化されているため、分析や学習のたびに ROS bag を再パースし、TF ツリーを組み直し、そろっていないクロックを合わせ直すところから始まります。LeRobot のような学習形式は読み込みの問題こそ解決しますが、設計上「非可逆」です。どのトピック・どのレート・どの特徴量を使うかという選択を固定してしまうため、考えを変えるたびに ROS bag からやり直すことになります。
...
> [!NOTE]
> 各 ROS bag には小さな `meta.json` サイドカー（データセット ID と、フルパイプライン用のセグメントラベル）が必要です。同梱の設定ファイルはこれを前提にしています。詳しくは [docs/metadata_ja.md](docs/metadata_ja.md) を参照してください。
...
**学習の準備ができたら、LeRobot v2.1 データセットを作ります。** 1 つの宣言的なパイプラインで、ロボットごとのコードは不要。`export` と同じく、ディレクトリをまとめて並列に処理できます:
...
新しいロボットへの対応は YAML 1 枚です。ROS のトピックとフィールドパスを LeRobot の特徴量に対応づけます。
...
- **[ガイド](docs/guide_ja.md)** — 新しいロボットのデータセットを作る
- **[CLI](docs/cli_ja.md)** — `run`・`export`・`merge`
- **[設定](docs/configuration_ja.md)** — パイプラインとロボットモデル
- **[エンコード](docs/encoding_ja.md)** — RGB・深度のコーデック設定
- **[メタデータ](docs/metadata_ja.md)** — `meta.json` サイドカー
- **[中間フォーマット](docs/intermediate-format_ja.md)** — Parquet + 動画のレイアウト
